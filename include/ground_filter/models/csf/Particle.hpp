...
{
public:
    Particle(const double& acceleration, const double time_step);

    Eigen::Vector2d GetPosition(){return position_;}
    void SetPosition(const Eigen::Vector2d& position){position_=position;}

    Eigen::Vector3d GetPosition3D(){return Eigen::Vector3d(position_[0], position_[1], (double)neighbors_.size());}
...
public:
    ...
    double             curr_height_;
    double             last_height_;
    std::vector<int>   corresponding_lidar_points_;
